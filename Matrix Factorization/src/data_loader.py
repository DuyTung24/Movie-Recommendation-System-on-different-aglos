...
class MovieLensDataLoader:
    ...
    def get_train_test_split(self, test_size=0.2, random_state=42):
        ...
        Trả về dưới dạng NumPy arrays (user_idx, item_idx, rating) để tăng tốc độ duyệt (iteration) khi train.
        """
        if self.dataset is None:
            raise ValueError("Dữ liệu chưa được load. Vui lòng gọi hàm load_and_preprocess() trước.")
        
        print(f"Đang chia tập dữ liệu với test_size = {test_size}...")
        ...
        # Chuyển đổi DataFrame sang NumPy array để thuật toán tối ưu (như SGD) chạy nhanh hơn
        train_data = train_df.values
        test_data = test_df.values
        
        print(f"Số lượng sample tập Train: {len(train_data)}")
        print(f"Số lượng sample tập Test: {len(test_data)}")
...
# Test nhanh script khi chạy trực tiếp file này
if __name__ == "__main__":

    FILE_PATH = "../data/raw/ml-1M/ratings.dat" 
    
    try:
        loader = MovieLensDataLoader(FILE_PATH)
        loader.load_and_preprocess()
        train_data, test_data = loader.get_train_test_split(test_size=0.2)
        
        # In thử 3 dòng đầu tiên của tập train
        print("\n3 dòng đầu tiên của tập Train (user_idx, item_idx, rating):")
        print(train_data[:3])
