...
def main():
    # =========================================================================
    # BƯỚC 1: THU THẬP THAM SỐ VÀ NẠP CONFIGS
    # =========================================================================
    args = parse_arguments()
    
    # Tạo đường dẫn động dựa trên mô hình được chọn, ví dụ: configs/gmf_config.yaml
    ...
    print("[1/3] Đang phân tách tập dữ liệu Test phục vụ giao thức kiểm toán RANDOM 80/20...")
    try:
        test_ratings = np.load(os.path.join(processed_dir, 'test_ratings.npy'), allow_pickle=True).item()
        test_negatives = np.load(os.path.join(processed_dir, 'test_negatives.npy'), allow_pickle=True).item()
        print(f"    -> Đã nạp xong tập Test cho {len(test_ratings)} users.")
    ...
    # =========================================================================
        ...
        #Chạy đánh giá toàn diện lại các mốc chỉ số Top-K dựa theo cấu hình mốc K trong file config
        ...
        # Demo tính toán dự đoán thực tế cho một User bất kỳ để chứng minh với hội đồng
