...
class MLP(nn.Module):
    ...
    def _init_weights(self):
        ...
        # Các tầng ẩn MLP: Do sử dụng hàm kích hoạt ReLU, việc khởi tạo He/Kaiming Uniform là bắt buộc để tránh hiện tượng Vanishing Gradient.
    ...
    def forward(self, user_indices, item_indices):
        ...
        """
        #  Trích xuất vector đặc trưng từ tầng nhúng
        user_embed = self.user_embedding(user_indices)
        item_embed = self.item_embedding(item_indices)
        
        # Concatenate hai vector lại với nhau.
        # Đây là điểm KHÁC BIỆT CỐT LÕI so với GMF.
        # Kết quả sẽ có kích thước: embed_size + embed_size = layers[0]
        vector_concat = torch.cat([user_embed, item_embed], dim=-1)
        
        # Đẩy vector đã ghép qua toàn bộ cấu trúc tháp MLP (gồm các tầng Linear + ReLU)
        hidden_output = self.mlp_network(vector_concat)
        
        # Đẩy qua tầng cuối cùng tính Logit, bọc Sigmoid
