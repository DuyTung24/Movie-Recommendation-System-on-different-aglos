...
class GMF(nn.Module):
    ...
    def __init__(self, num_users, num_items, num_factors):
        ...
        # Tương đương với layer: Dense(1, activation='sigmoid', ...) trong Keras của tác giả
        self.prediction_layer = nn.Linear(num_factors, 1)
        
        # 3. Hàm kích hoạt Hàm mũ Sigmoid để chuẩn hóa đầu ra về khoảng xác suất [0, 1] cho bài toán Implicit Feedback
        self.sigmoid = nn.Sigmoid()
        
        # Kích hoạt hàm khởi tạo trọng số mặc định chuẩn học thuật
        self._init_weights()

    def _init_weights(self):
        """
        Khởi tạo trọng số ngẫu nhiên ban đầu cho các tham số mạng.
        Bám sát chính xác thiết lập của tác giả Xiangnan He: Phân phối chuẩn với std=0.01 cho Embedding.
        """
        nn.init.normal_(self.user_embedding.weight, std=0.01)
        nn.init.normal_(self.item_embedding.weight, std=0.01)
        
        
        # Khởi tạo chuẩn theo paper cho lớp dự đoán
    ...
    def forward(self, user_indices, item_indices):
        ...
        """
        # Bước A: Trích xuất vector ẩn từ tầng nhúng dựa trên ID đầu vào
        user_embed = self.user_embedding(user_indices)
        item_embed = self.item_embedding(item_indices)
        
        # Bước B: Thực hiện phép nhân từng phần tử (Element-wise Product / Hadamard Product)
        ...
        # Bước C: Đẩy qua tầng dự đoán để áp công thức tính tổng trọng số tuyến tính h^T
        logits = self.prediction_layer(element_product)
        
        # Bước D: Áp dụng hàm Sigmoid và làm phẳng mảng (Flatten) về dạng 1D tương thích với hàm Loss (BCELoss)
