...
class MemoryBasedCF:
    ...
    def predict_for_user_items(self, u_idx, item_indices, utility_sparse, utility_norm_sparse, user_means, item_means, utility_csc=None, utility_norm_csc=None):
        """
        Dự đoán Rating theo yêu cầu (On-demand/Lazy Prediction).
        Chỉ tính toán rating cho danh sách item cụ thể của user cụ thể để tối ưu bộ nhớ.
        """
        ...
        if self.mode == 'item':
            ...
            if len(rated_items) == 0:
                fallback_val = item_means[item_indices]
                fallback_val[fallback_val == 0] = self.global_mean 
                return fallback_val
